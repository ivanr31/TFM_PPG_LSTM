...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Codigo para leer los ficheros terminados en _1:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Codigo para leer los ficheros terminados en _3:
...
% Definir parámetros
num_nuevas_series = 15000; % Número de nuevas series temporales a generar
...
% Parámetros
num_features = 2100 % Número de características en tus datos de entrada
num_hidden_units = 5; % Número de unidades ocultas en la capa LSTM
...
lstmModel = trainnet(X_entrenamiento, Y_entrenamiento, layers,"crossentropy", options);
...
scores = minibatchpredict(lstmModel,X_validacion,MiniBatchSize=75);
YTest = scores2label(scores,categories(categorical(Y_validacion)));
acc = sum(YTest == categorical(Y_validacion))./numel(Y_validacion)


disp(['Precisión del modelo en datos de prueba: ', num2str(acc)]);

[m,order]=confusionmat(categorical(Y_validacion),YTest);
